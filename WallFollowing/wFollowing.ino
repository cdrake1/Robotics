...
const int HEAD_POSITIONS[NUM_HEAD_POSITIONS] = { 0, 10, 20 };
int angle;
...
const double desiredDistance = (double)30.5;
...
void moveHead() {
  ...
  if (headCm > headPm + HEAD_MOVEMENT_PERIOD) {
    ...
    angle = HEAD_POSITIONS[currentHeadPosition] * (180 / 3.14);
...
void setMotors(float leftSpeed, float rightSpeed) {
  ...
  if (motorCm > motorPm + MOTOR_PERIOD) {
    ...
    // if(MOTOR_MIN_SPEED < leftSpeed && leftSpeed <= 0) leftSpeed = MOTOR_MIN_SPEED;
    // if(-MOTOR_MIN_SPEED > leftSpeed && leftSpeed > 0) leftSpeed = -MOTOR_MIN_SPEED;
    // if(MOTOR_MIN_SPEED < rightSpeed && leftSpeed <= 0) rightSpeed = MOTOR_MIN_SPEED;
    // if(-MOTOR_MIN_SPEED > rightSpeed && leftSpeed > 0) rightSpeed = -MOTOR_MIN_SPEED;
