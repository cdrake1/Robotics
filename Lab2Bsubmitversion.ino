...
//an object using an Ultrasonic Sensor.
//By Ryan Rosenkranse, Nick Suchy, Collin Drake
//Version: Lab2B
...
const int HEAD_POSITIONS[NUM_HEAD_POSITIONS] = {105, 90, 75};
...
// Motor constants
const float MOTOR_BASE_SPEED = 150.0;
...
void usReadCm() {
  ...
  if(usCm > usPm + US_PERIOD) {
    ...
    float duration = pulseIn(ECHO_PIN, HIGH, 38000);
...
void setMotors() {
  ...
  if(distance <= MAX_DISTANCE) {
    ...
    // Dynamics Contol Tuning
    // if distance of object is between 7 and 5 then the robot begins to deaccelerate
    // makes sure that speed does not go lower than 30.
    if(distance <= 7 && distance >= 5){
      leftSpeed= (10 * (abs((distance-leftSpeed)) / 10));
      rightSpeed= (10 * (abs((distance-rightSpeed)) / 10));
